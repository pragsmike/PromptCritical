(ns pcrit.log.log
  ...
  * Initialises a single console handler on first require; safe to require from
  ...
  | Var | Purpose | Example |
  |------|---------|---------|
  | `PCRIT_LOG_TS_PATTERN` | DateTimeFormatter pattern | `yyyy-MM-dd HH:mm:ss.SSS` |
  | `PCRIT_LOG_TZ`         | IANA/Java TZ ID            | `America/New_York` |
  | `PCRIT_LOG_HANDLER`    | Handler key (rare)         | `:console-2` |
...
(defn init!
  ...
  ```"
  ([] (init! {}))
  ([{:keys [handler-id pattern zone] :or {handler-id (keyword (env-or "PCRIT_LOG_HANDLER" "console"))}}]
   (when-not @initialized?
     (let [pattern'  (or pattern (env-or "PCRIT_LOG_TS_PATTERN" default-ts-pattern))
           zone'     (->zone-id (or zone (System/getenv "PCRIT_LOG_TZ")))
